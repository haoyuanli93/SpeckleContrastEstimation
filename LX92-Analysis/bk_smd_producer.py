...
def getROIs(run):
    ...
    if run == 570:
        roi_dict = {}
        roi_dict['ROIs'] = [[[0, 500], [0, 2000]]]  # can define more than one ROI
        roi_dict['writeArea'] = True
        roi_dict['thresADU'] = None
        ret_dict['zyla_0'] = roi_dict
    return ret_dict
...
def define_dets(run):
    detnames = ['jungfrau1M', 'epix_alc1', 'epix_alc2', 'epix_alc3', 'epix_alc4', 'zyla_0']  # add detector here
    dets = []
    ...
    for detname in detnames:
        ...
        if havedet:
            ...
            if detname in drop2phot:
                ...
                print(d2p_dict)
                # instantiate the droplet + droplet-to-photon analysis chain
                dropfunc = dropletFunc(**droplet_dict)
                drop2phot_func = droplet2Photons(**d2p_dict)
                sparsify = sparsifyFunc(nData=nData)
                # sparsify returns the list of photon and positions
...
for evt_num, evt in enumerate(ds.events()):
    ...
    if ((evt_num < 100 and evt_num % 10 == 0) or (evt_num < 1000 and evt_num % 100 == 0) or (evt_num % 1000 == 0)):
        if os.environ.get('ARP_JOB_ID', None) is not None:
            if ds.size == 1:
                requests.post(os.environ["JID_UPDATE_COUNTERS"],
                              json=[{"key": "<b>Current Event</b>", "value": evt_num + 1}])
            elif ds.rank == 0:
                requests.post(os.environ["JID_UPDATE_COUNTERS"],
                              json=[{"key": "<b>Current Event / rank </b>", "value": evt_num + 1}])
        else:
            if ds.size == 1:
...
if os.environ.get('ARP_JOB_ID', None) is not None:
    if ds.size > 1:
        if ds.rank == 0:
            requests.post(os.environ["JID_UPDATE_COUNTERS"],
                          json=[{"key": "<b>Last Event</b>", "value": "~ %d cores * %d evts" % (ds.size, evt_num)},
